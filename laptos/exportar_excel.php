...
try {
    // Capturar los IDs filtrados enviados por JavaScript
    $ids_filtrados = $_POST['ids_filtrados'] ?? '';

    // Consulta base (Campos requeridos)
    $campos = "
        id_local,
        serie,
        equipo_marca,
    ...
    // 1. EVALUAR SI HAY FILTROS APLICADOS EN LA PANTALLA
    if (!empty($ids_filtrados)) {
        // Convertimos el string separado por comas en un Array de PHP
        $array_ids = explode(',', $ids_filtrados);
        
        // Generamos tantos signos de interrogación como elementos existan en el array: (?, ?, ?)
        $placeholders = implode(',', array_fill(0, count($array_ids), '?'));
        
        // Filtramos la base de datos de manera segura usando exclusivamente esos IDs
        $sql = "SELECT $campos FROM productos_informatica WHERE id_local IN ($placeholders) ORDER BY equipo_marca ASC";
        $stmt = $pdo->prepare($sql);
        $stmt->execute($array_ids);
    } else {
        // Si el string está vacío, el usuario quiere el reporte general de lo que no esté vendido
        $sql = "SELECT $campos FROM productos_informatica WHERE estado != 'Vendida' ORDER BY equipo_marca ASC";
        $stmt = $pdo->query($sql);
    }

    $equipos = $stmt->fetchAll(PDO::FETCH_ASSOC);

    // Configuración de Cabeceras HTTP para la descarga del archivo Excel (.xls)
    header("Content-Type: application/vnd.ms-excel; charset=UTF-8");
    header("Content-Disposition: attachment; filename=inventario_dacans.xls");
    header("Pragma: no-cache");
    header("Expires: 0");

    // Imprimir el BOM UTF-8 para forzar a Excel a leer los acentos y caracteres especiales correctamente
    echo "\xEF\xBB\xBF";
