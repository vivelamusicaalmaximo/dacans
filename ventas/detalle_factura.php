...
<body class="bg-slate-50 text-slate-800 font-sans antialiased min-h-screen">
    ...
    <div class="max-w-5xl mx-auto px-4 py-8">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
            <div>
                <a href="facturas.php" class="inline-flex items-center text-sm font-medium text-blue-600 hover:text-blue-800 transition">
            ...
            <div class="flex items-center gap-3">
                <a href="generar_pdf_factura.php?id=<?php echo $id_factura; ?>" target="_blank" class="inline-flex items-center justify-center bg-blue-600 hover:bg-blue-700 text-white font-medium px-4 py-2.5 rounded-lg shadow-sm text-sm transition">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
            ...
            <div class="p-6 sm:p-8 border-b border-slate-100 bg-slate-900 text-white flex flex-col sm:flex-row sm:items-center sm:justify-between gap-6">
                <div class="flex items-center gap-4">
                    <div class="bg-white p-2 rounded-xl border border-slate-700 w-16 h-16 flex items-center justify-center">
                        <img src="https://dacansdr.com/img/logo.png" alt="DACANS Logo" class="max-h-12 w-auto object-contain">
                    </div>
                    <div>
                ...
                <div class="sm:text-right">
                    ...
                    <div class="text-2xl font-mono font-bold text-blue-400"><?php echo htmlspecialchars($factura['numero_factura']); ?></div>
                    <div class="mt-2 flex sm:justify-end">
                        <?php if ($factura['estado_factura'] === 'PAGADA'): ?>
                            <span class="inline-flex items-center bg-emerald-500/10 text-emerald-400 text-xs font-bold px-3 py-1 rounded-full border border-emerald-500/20">
                                <span class="w-1.5 h-1.5 rounded-full bg-emerald-400 mr-1.5 animate-pulse"></span> PAGADA
                            </span>
                        <?php else: ?>
                            <span class="inline-flex items-center bg-rose-500/10 text-rose-400 text-xs font-bold px-3 py-1 rounded-full border border-rose-500/20">
            ...
            <div class="p-6 sm:p-8 border-b border-slate-100 bg-slate-50 grid grid-cols-1 sm:grid-cols-3 gap-6 text-sm">
                <div>
                    <span class="text-xs text-slate-400 font-bold uppercase tracking-wider block mb-1">Información del Cliente</span>
                    <div class="font-bold text-slate-900 text-base"><?php echo htmlspecialchars($factura['cliente_nombre']); ?></div>
                    <div class="text-slate-500 mt-1"><span class="font-medium text-slate-600">Condición:</span> Contado</div>
                </div>
                
                <div>
                    <span class="text-xs text-slate-400 font-bold uppercase tracking-wider block mb-1">Detalles del Pago</span>
                    <div class="text-slate-700 mt-0.5"><span class="font-semibold text-slate-600">Método:</span> <?php echo htmlspecialchars($factura['nombre_metodo']); ?></div>
                    <div class="text-slate-700 mt-0.5"><span class="font-semibold text-slate-600">Envío:</span> <?php echo $agencia_envio; ?></div>
                </div>
                
                <div class="sm:text-right">
                    <span class="text-xs text-slate-400 font-bold uppercase tracking-wider block mb-1">Fecha de Emisión</span>
                    <div class="text-slate-800 font-semibold text-base"><?php echo date('d/m/Y', strtotime($factura['fecha_factura'])); ?></div>
                    <div class="text-xs text-slate-500 mt-0.5"><?php echo date('h:i A', strtotime($factura['fecha_factura'])); ?></div>
                </div>
            </div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-slate-900 text-slate-200 text-xs font-bold uppercase tracking-wider border-b border-slate-200">
            ...
            <div class="p-6 sm:p-8 bg-slate-50/50 border-t border-slate-100 flex flex-col md:flex-row gap-8 justify-between items-start">
                
                <div class="w-full md:w-auto max-w-sm bg-white p-5 rounded-2xl border-2 border-dashed border-slate-200 shadow-sm flex items-center gap-4">
                    <div class="p-3 bg-blue-50 text-blue-600 rounded-xl flex-shrink-0">
                        <img src="../img/garantia.png" alt="Sello de Garantía" class="w-10 h-10 object-contain" onerror="this.src='https://cdn-icons-png.flaticon.com/512/9722/9722912.png';">
                    </div>
                    <div>
                        <h4 class="text-sm font-bold text-slate-900 uppercase tracking-wide">1 Año de Garantía</h4>
                        <p class="text-xs text-slate-500 mt-0.5 leading-relaxed">Este documento valida la cobertura oficial de hardware exclusiva de DACANS COMPUTERS.</p>
                    </div>
                </div>
                ...
                <div class="w-full md:w-80 space-y-3 text-sm">
                    ...
                    <div class="pt-3 border-t-2 border-slate-200 flex justify-between items-baseline">
                        ...
                        <span class="text-xl font-black text-emerald-600">RD$ <?php echo number_format($total_final, 2); ?></span>
                    </div>
                </div>
