...
/* ======================================================
   FILTROS INDEPENDIENTES DESDE LA DB (SIEMPRE VISIBLES)
====================================================== */
...
/* ======================================================
   CATEGORÍAS ACTIVAS (SOLO SI TIENEN REGISTROS EN 'Lista')
====================================================== */
try {
    // Esta consulta busca todas las categorías, pero el EXISTS asegura 
    // que solo traiga aquellas que tengan mínimo 1 producto asociado en estado 'Lista'.
      $stmtCatList = $pdo->query("
...
if (!empty($equipo_sel)) {
    ...
    foreach ($equipo_sel as $v) {
        $condiciones[] = "REPLACE(UPPER(p.equipo_marca),' ','') LIKE ?";
...
if (!empty($proc_sel)) {
    ...
    foreach ($proc_sel as $v) {
        $condiciones[] = "REPLACE(UPPER(p.proc_marca),' ','') LIKE ?";
        $params[] = '%' . limpiarValor($v) . '%';
    }
    $condiciones_dinamicas .= " AND (" . implode(' OR ', $condiciones) . ")";
}
...
if (!empty($ram_sel)) {
    ...
    foreach ($ram_sel as $v) {
        $condiciones[] = "REPLACE(UPPER(p.memoria),' ','') LIKE ?";
        $params[] = '%' . limpiarValor($v) . '%';
    }
    $condiciones_dinamicas .= " AND (" . implode(' OR ', $condiciones) . ")";
}
...
if (!empty($disco_sel)) {
    ...
    foreach ($disco_sel as $v) {
        $condiciones[] = "REPLACE(UPPER(p.disco),' ','') LIKE ?";
        $params[] = '%' . limpiarValor($v) . '%';
    }
    $condiciones_dinamicas .= " AND (" . implode(' OR ', $condiciones) . ")";
}
...
if (!empty($gens_sel)) {
    ...
    foreach ($gens_sel as $v) {
        $condiciones[] = "REPLACE(UPPER(p.proc_generacion),' ','') LIKE ?";
        $params[] = '%' . limpiarValor($v) . '%';
    }
    $condiciones_dinamicas .= " AND (" . implode(' OR ', $condiciones) . ")";
}
...
if (!empty($pantalla_sel)) {
    ...
    foreach ($pantalla_sel as $v) {
        $condiciones[] = "REPLACE(UPPER(p.pantalla),' ','') LIKE ?";
        $params[] = '%' . limpiarValor($v) . '%';
    }
    $condiciones_dinamicas .= " AND (" . implode(' OR ', $condiciones) . ")";
}
...
/* ======================================================
   ENSAMBLAJE DE LA QUERY FINAL CON CTE
====================================================== */
// Corregido: Agregamos p.id_categoria a la partición para que no mezcle el modelo 
// de una serie/categoría distinta y las oculte erróneamente.
$query_final = "
            ...
            <div class="bg-white rounded-3xl border border-slate-200 shadow-sm p-4 lg:p-5 mb-6">
                ...
                <div class="mt-6 pt-6 border-t border-slate-200">
                    ...
                    <div class="flex items-center gap-2 mb-4">
                        ...
                        <span class="font-black text-slate-700 uppercase tracking-wide">
                            Línea / Categoría
                        </span>
                    </div>
                    ...
                    <form method="GET">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
                            ...
                            <label class="cursor-pointer">
                                ...
                                <div
                                    class="h-full rounded-2xl border border-slate-200 bg-white p-4 transition-all duration-200 hover:border-blue-500 hover:shadow-lg hover:-translate-y-1 peer-checked:border-blue-600 peer-checked:bg-gradient-to-br peer-checked:from-blue-600 peer-checked:to-cyan-600 peer-checked:text-white">

                                    <div class="flex items-start justify-between gap-3">

                                        <div class="flex-1 min-w-0">

                                            <h4 class="font-black text-base leading-tight">
                                                <?= htmlspecialchars($cat['nombre_serie']) ?>
                                            </h4>

                                            <?php if (!empty($cat['rango_uso'])): ?>
                                            <p class="text-xs mt-2 opacity-80 line-clamp-3">
                                                <?= htmlspecialchars($cat['rango_uso']) ?>
                                            </p>
                                            <?php endif; ?>

                                        </div>

                                        <?php if (!empty($cat['prefijo'])): ?>
                                        <span
                                            class="px-2 py-1 rounded-lg bg-slate-100 text-slate-700 text-[10px] font-black uppercase tracking-wide flex-shrink-0 peer-checked:bg-white/20 peer-checked:text-white">
                                            <?= htmlspecialchars($cat['prefijo']) ?>
                                        </span>
                                        <?php endif; ?>

                                    </div>

                                    <?php if (!empty($cat['regaleria_promocion'])): ?>
                                    <div
                                        class="mt-3 flex items-center gap-2 rounded-xl bg-amber-50 border border-amber-200 p-2">

                                        <i class="fa-solid fa-gift text-amber-500"></i>

                                        <span class="text-[11px] font-bold text-amber-700 line-clamp-2">
                                            <?= htmlspecialchars($cat['regaleria_promocion']) ?>
                                        </span>
                                    ...
                                    <div class="mt-3">
                                        ...
                                        <?php if (in_array($cat['id_categoria'], $categorias_sel)): ?>

                                        <span
                                            class="inline-flex items-center gap-1 text-[10px] font-black uppercase bg-white/20 px-2 py-1 rounded-lg">
                                            ✓ Seleccionada
                                        </span>
                                        ...
                                        <span
                                            class="inline-flex items-center gap-1 text-[10px] font-black uppercase text-slate-400">
                                            Clic para filtrar
                                        </span>
...
if (strpos($familia, 'i3') !== false) {
    $img_procesador = 'https://dacansdr.com/img/i3.png';
